...
const config = useRuntimeConfig().public.Nuxt3EditorJS.Api.PersonalityTool as ApiPersonalityToolOptions
...
export default defineEventHandler(async (event) => {
  ...
  try {
    await callNodeListener(upload.single('image') as NodeMiddleware, event.node.req, event.node.res)
